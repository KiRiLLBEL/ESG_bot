...
async def create_task_themes(callback: CallbackQuery,  state: FSMContext, bot: Bot, session: AsyncSession):
...
async def create_task_themes(callback: CallbackQuery,  state: FSMContext, bot: Bot):
...
async def error_message(message: Message, state: FSMContext, bot: Bot):
    data = await state.get_data()
    msg = data.get('msg')
    msg = await bot.edit_message_text(chat_id=message.from_user.id, message_id=msg, text='Вы должны ввести текст\n\nВведите название товара (не длиннее 64 символов)')
...
async def create_task_add_final_error(message: Message,  state: FSMContext, bot: Bot):
    data = await state.get_data()
    msg = data.get('msg')
    msg = await bot.edit_message_text(chat_id=message.from_user.id, message_id=msg, text='Введите число')
    data['msg'] = msg.message_id
    await state.set_data(data)
    await message.delete()
...
async def get_task_themes(callback: CallbackQuery,  state: FSMContext, bot: Bot, session: AsyncSession):
...
async def change_menu_keyboard(callback: CallbackQuery, callback_data: AdminMenuCallbackFactory,  state: FSMContext, bot: Bot):
...
async def get_task_for_delete(callback: CallbackQuery,  state: FSMContext, bot: Bot, session: AsyncSession):
...
async def delete_task(callback: CallbackQuery,  state: FSMContext, bot: Bot, session: AsyncSession):
...
async def get_task_names(callback: CallbackQuery,  state: FSMContext, bot: Bot, session: AsyncSession):
    tasks = await get_tasks_by_theme(session, callback.data)
    if not tasks:
        await callback.answer(text='Нет созданных заданий')
        return
...
async def get_task_uid(callback: CallbackQuery,  state: FSMContext, bot: Bot, session: AsyncSession):
    uid = await get_tasks_uid_by_name(session, callback.data)
...
async def create_task_add_theme(callback: CallbackQuery,  state: FSMContext, bot: Bot):
...
async def create_task_pick_theme(callback: CallbackQuery,  state: FSMContext, bot: Bot):
...
async def create_poll_add_name(message: Message,  state: FSMContext, bot: Bot, session: AsyncSession):
    ...
    if await theme_exists_pool(session, message.text):
        await bot.edit_message_text(chat_id=message.from_user.id, message_id=msg, text='Тема уже существует. Введите другую тему')
        await message.delete()
        return
    await state.set_state(Admin.pool_name)
    await state.set_data({"theme": message.text, "msg": msg})
    await bot.edit_message_text(chat_id=message.from_user.id, message_id=msg, text='Введите название опроса (не длиннее 64 символов)')
    await message.delete()
...
async def create_task_add_name(message: Message,  state: FSMContext, bot: Bot, session: AsyncSession):
    ...
    await state.set_state(Admin.task_name)
    await state.set_data({"theme": message.text, "msg": msg})
    await bot.edit_message_text(chat_id=message.from_user.id, message_id=msg, text='Введите название задачи (не длиннее 64 символов)')
    await message.delete()
...
async def error_message(message: Message,  state: FSMContext, bot: Bot):
    data = await state.get_data()
    msg = data.get('msg')
    await bot.edit_message_text(chat_id=message.from_user.id, message_id=msg, text='Вы должны ввести текст\n\nВведите название опроса (не длиннее 64 символов)')
    await message.delete()
...
async def create_pool_add_score(message: Message,  state: FSMContext, bot: Bot):
    ...
    await message.delete()
    await bot.edit_message_text(chat_id=message.from_user.id, message_id=msg, text='Введите вопрос', reply_markup=callback_map_admin['poll_question'])
...
async def create_task_add_final_error(message: Message,  state: FSMContext, bot: Bot):
    data = await state.get_data()
    msg = data.get('msg')
    await bot.edit_message_text(chat_id=message.from_user.id, message_id=msg, text='Введите число')
    await message.delete()
...
async def create_question(message: Message,  state: FSMContext, bot: Bot):
    ...
    await state.set_data(data)
    await bot.edit_message_text(chat_id=message.from_user.id, message_id=msg, text="Введите текст варианта ответа или нажмите кнопку завершить, чтобы закончить вопрос",
...
async def questions_text(callback: CallbackQuery,  state: FSMContext, bot: Bot, session: AsyncSession):
    data = await state.get_data()
    print(data)
    msg = data.get('msg')
    if not data['current_options']:
        ...
        await callback.message.edit_text(text=LEXICON_RU['admin']['menu']['message'], reply_markup=callback_map_admin['menu'])
...
async def options_text(message: Message,  state: FSMContext, bot: Bot):
    ...
    if not data['current_options']:
        await bot.edit_message_text(chat_id=message.from_user.id, message_id=msg, 
            text="Введите текст следующего варианта ответа или нажмите кнопку завершить, чтобы закончить вопрос",
            reply_markup=callback_map_admin['poll_option'])
...
async def end_options(callback: CallbackQuery,  state: FSMContext, bot: Bot):
    data = await state.get_data()
    print(data)
    if not data['current_options']:
        await callback.message.edit_text(text="Вопрос должен иметь хотя бы один вариант ответа. Введите текст варианта ответа:",
                            reply_markup=callback_map_admin['poll_option'])
    else:
        await callback.message.edit_text(text="Введите текст следующего вопроса нажмите кнопку завершить, чтобы закончить опрос:",
                            reply_markup=callback_map_admin['poll_question'])
        await state.set_state(Admin.pool_questions)
        await state.set_data(data)
...
async def create_task_add_score(message: Message,  state: FSMContext, bot: Bot):
    ...
    await bot.edit_message_text(chat_id=message.from_user.id, message_id=msg, text='Введите количество баллов за выполнение задачи')
    await message.delete()
...
async def create_task_add_final(message: Message,  state: FSMContext, bot: Bot, session: AsyncSession):
    ...
    await state.set_state(Admin.join)
    await bot.edit_message_text(chat_id=message.from_user.id, message_id=msg, text=f'Задача успешно создана. UID = {uid}', reply_markup=callback_map_admin['menu'])
    await message.delete()
